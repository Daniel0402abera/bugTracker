...
export default function BugSelection({ onSelectedBugChange }) {
  const {
    data,
  } = useGet("/api/v1/bugs");
  

  const options = data?.map(bug => ({ id: bug?.bugId, label: bug?.title })); // Map data to include both ID and label
  ...
  return (
    <Autocomplete
      ...
      renderInput={(params) => <TextField {...params} label="Select Bug To Assign From The List" />}
